...
async function queryNUT(host, port, upsName, username, password, vars, timeoutMs = 8000) {
  ...
  // Every ERR line present was 'unsupported' (or there were none), so no
  // structured error surfaced above — but if that left the parsed data
  // empty, every single requested variable came back VAR-NOT-SUPPORTED (or
  // a sibling unsupported code). Resolving with {} here would reproduce
  // exactly the failure this client exists to prevent: a "successful" query
  // with blank telemetry and no indication anything is wrong (see #237,
  // #241). A driver that supports none of the requested variables is a
  // configuration problem, not something a retry will fix.
  if (Object.keys(data).length === 0 && allErrors.length > 0) {
